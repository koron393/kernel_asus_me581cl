...
static int watchdog_set_timeouts_for_reboot_notifier(void)
{
	int pre_timeout_for_reboot = 5;
	int timeout_for_reboot = 10;
	pr_info("%s\n", __func__);
	pr_info("Set pretimeout:%d\ttimeout:%d\n", pre_timeout_for_reboot, timeout_for_reboot);
	return watchdog_set_timeouts_and_start(pre_timeout_for_reboot, timeout_for_reboot);
}
			   ...
			   void *another_unused)
{
	...
	if (code == SYS_RESTART || code == SYS_HALT || code == SYS_POWER_OFF) {
		...
		if (code == SYS_RESTART) {
			if (watchdog_set_timeouts_for_reboot_notifier())

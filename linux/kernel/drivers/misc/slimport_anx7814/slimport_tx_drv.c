...
EXPORT_SYMBOL(downstream_charging_status_get);
			...
			if (AUX_OK ==
				...
				if ((data_buf[0] == 0xb9)
					...
					if ((data_buf[0] == 0x00)
						...
						down_sample_en = 1;
						pr_info(
						"%s %s : Downstream is HDMI 4K dongle.\n",
						     LOG_TAG, __func__);
					...
					cur_cable_type = DWN_STRM_IS_DIGITAL;
					pr_info(
					"%s %s : Downstream is general DP2HDMI converter.\n",
					     LOG_TAG, __func__);
				}
			} else
...
void slimport_link_training(void)
{
	...
	case LT_ERROR:
		...
		pr_debug("LT ERROR Status: SERDES FIFO reset.");

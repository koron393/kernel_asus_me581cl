...
static int intel_moor_sd_probe_slot(struct sdhci_pci_slot *slot)
{
	...
	if (!gpio_is_valid(MOOR_SD_PWR_EN))
                return -ENODEV;
	slot->host->gpio_pwr_en = MOOR_SD_PWR_EN;

	ret = gpio_request(slot->host->gpio_pwr_en, "sd_power");
	if (ret < 0)
		goto out;

	ret = gpio_direction_output(slot->host->gpio_pwr_en, 0);
	if (ret < 0)
		goto free;

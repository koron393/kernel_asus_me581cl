...
int mmc_debug_enable;
module_param(mmc_debug_enable, int, 0644);
int sd_debug_enable;
module_param(sd_debug_enable, int, 0644);

#define DEBUG_REQ_START        BIT(0)
#define DEBUG_REQ_DONE         BIT(1)
...
void mmc_request_done(struct mmc_host *host, struct mmc_request *mrq)
{
	...
	print_log = ((mmc_debug_enable & DEBUG_REQ_DONE)
		&& !strcmp("mmc0", mmc_hostname(host)))
		|| ((sd_debug_enable & DEBUG_REQ_DONE)
		&& !strcmp("mmc1", mmc_hostname(host))) ? 1 : 0;
...
mmc_start_request(struct mmc_host *host, struct mmc_request *mrq)
{
	...
	print_log = ((mmc_debug_enable & DEBUG_REQ_START)
		&& !strcmp("mmc0", mmc_hostname(host)))
		|| ((sd_debug_enable & DEBUG_REQ_START)
		&& !strcmp("mmc1", mmc_hostname(host))) ? 1 : 0;

	if (mrq->sbc && print_log) {
...
static void mmc_power_up(struct mmc_host *host)
{
	...
	if(!strcmp("mmc1", mmc_hostname(host)) && sdhci->gpio_pwr_en
		&& gpio_is_valid(sdhci->gpio_pwr_en)
		&& !gpio_get_value(sdhci->gpio_pwr_en)) {
		gpio_set_value(sdhci->gpio_pwr_en, 1);
	}
...
void mmc_power_off(struct mmc_host *host)
{
	...
	if(!strcmp("mmc1", mmc_hostname(host)) && sdhci->gpio_pwr_en
		&& gpio_is_valid(sdhci->gpio_pwr_en)
		&& gpio_get_value(sdhci->gpio_pwr_en))
		gpio_set_value(sdhci->gpio_pwr_en, 0);

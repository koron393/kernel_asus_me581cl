...
int IFWI_minor_version;
EXPORT_SYMBOL(IFWI_minor_version);
...
static void read_ifwi_version(void)
{
	...
	if (ifwi_rev_ext) {
		...
		IFWI_minor_version = version.data[FW_VERSION_SIZE+1] << 8 |
					version.data[FW_VERSION_SIZE];
		pr_info("IFWI_minor_version=%x\n", IFWI_minor_version);

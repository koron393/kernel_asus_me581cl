...
#define CMD_SETBCNTIMEOUT	"SETBCNTIMEOUT"
...
int wl_android_set_bcn_timeout(struct net_device *dev, char *command, int total_len)
{
	int error = 0;
	int timeout = 0;

	if (sscanf(command, "%*s %d", &timeout) != 1) {
		DHD_ERROR(("%s: Failed to get Parameter\n", __FUNCTION__));
		return -1;
	}

	error = wldev_iovar_setint(dev, "bcn_timeout", timeout);
	if (error) {
		DHD_ERROR(("%s: Failed to set BCN timeout %d, error = %d\n",
		__FUNCTION__, timeout, error));
		return -1;
        }
	else
		DHD_ERROR(("%s: succeeded to set BCN timeout %d, error = %d\n",
		__FUNCTION__, timeout, error));
	return 0;
}
...
#define PRIVATE_COMMAND_MAX_LEN	8192
	...
	else if (strnicmp(command, CMD_SETBCNTIMEOUT, strlen(CMD_SETBCNTIMEOUT)) == 0)

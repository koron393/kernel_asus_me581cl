...
static ssize_t store_moor_continue_dump(struct device *dev,
...
static int create_soc_dump_attr(struct thermal_data *tdata,
			struct device *dev)
{
...
{
	struct thermal_data *tdata =  container_of(work,
				struct thermal_data, dump_moor_temp.work);

					...
					struct devs_id *dev)
{
	...
	if(project == 0){
		/* hard-coded the bus of charger from 8 to 2 */
		if ((hw_version >= 0) && (hw_version <= 2)){
			if (!strncmp(pentry->name, "smb347_charger", 14)){
				pentry->host_num = 2;
				pr_info("smb347 changes to bus 2 due to hw_version = %d\n", hw_version);
			}
		} else if (hw_version >= 3){
			if (!strncmp(pentry->name, "smb347_charger", 14)){
				pentry->host_num = 8;
				pr_info("smb347 changes to bus 8 due to hw_version = %d\n", hw_version);
			}
		}
	}
...
static ssize_t asus_iafw_version_show(struct switch_dev *sdev, char *buf)
{
        return sprintf(buf, "%04d.%04d\n", temp_g_oemb->val_hooks_major_version,
                temp_g_oemb->val_hooks_minor_version);
}

...
module_param_named(hdmi_state, hdmi_state, int, 0644);
...
static void dump_sprites(struct drm_device *dev, int index)
{
       int i = 0;
       u32 reg_offset = 0;
       switch (index) {
       ...
       printk("\tSprites: reg_offset = 0x%x\n", reg_offset);
       for (i = 0x70180 + reg_offset; i < 0x701d4+reg_offset; i += 4)
               printk("\t\t reg(0x%x) = 0x%x\n", i, REG_READ(i));
}

static void dump_ova(struct drm_device *dev)
{
       int i = 0;
       printk("\tOverlay A:\n");
       for (i = 0x30000; i < 0x307ff; i += 4)
               printk("\t\t reg(0x%x) = 0x%x\n", i, REG_READ(i));
}

static void dump_dpll(struct drm_device *dev)
{
       int i = 0;
       printk("\tDPLL A:\n");
...
static void dump_pipea(struct drm_device *dev)
{
       int i = 0;
       printk("\tPipe A:\n");
       ...
       for (i = 0xb104; i < 0xb13b; i += 4)
               printk("\t\t reg(0x%x) = 0x%x\n", i, REG_READ(i));

       for (i = 0x60000; i < 0x600ff; i += 4)
               printk("\t\t reg(0x%x) = 0x%x\n", i, REG_READ(i));

       for (i = 0x61190; i < 0x61197; i += 4)
               printk("\t\t reg(0x%x) = 0x%x\n", i, REG_READ(i));
...
static void vsync_dump_regs(struct drm_device *dev)
{
       dump_sprites(dev, 0);
       dump_ova(dev);
       dump_dpll(dev);
		...
		if (arg->vsync_operation_mask & VSYNC_WAIT) {
			...
			if (dev_priv->vsync_enabled[pipe] && ((pipe == 1) ||
				...
				if (!dev_priv->vsync_enabled[pipe])
					DRM_ERROR(KERN_ERR "ERR: ASK for a VSYNC without enable.\n");
			}
...
static const struct dev_pm_ops psb_pm_ops = {
	...
	.suspend_late = rtpm_suspend_late_pm_transition,

...
static int mdm_ctrl_module_probe(struct platform_device *pdev)
{
	...
	mdm_drv->pdata->cpu.sim_det_init = sim_detect_init;
	mdm_drv->pdata->cpu.sim_det_exit = sim_detect_exit;
	mdm_drv->pdata->cpu.ap_req_cdump_init = ap_req_cdump_init;
	mdm_drv->pdata->cpu.ap_req_cdump_exit = ap_req_cdump_exit;

	mdm_drv->pdata->cpu.sim_det_init(mdm_drv, mdm_drv->pdata->cpu_data);
	mdm_drv->pdata->cpu.ap_req_cdump_init(mdm_drv, mdm_drv->pdata->cpu_data);
...
static int mdm_ctrl_module_remove(struct platform_device *pdev)
{
	...
	mdm_drv->pdata->cpu.ap_req_cdump_exit();
...
static int __init mdm_ctrl_module_init(void)
{
	...
	if (project == 2 || project == 7 || project == 5) {
		pr_info("%s: Wifi only version, don't init modem control driver\n", __func__);
		return -ENODEV;
	}

...
//---- Input Device
#define INPUT_DEV_NAME	"hx8528-e58"

//---- Flash dump file
#define FLASH_DUMP_FILE "/data/log/Touch_Flash_Dump.bin"
#define TOUCH_DC_DUMP_FILE "/data/log/touch/touch_dc.txt"
#define TOUCH_IIR_DUMP_FILE "/data/log/touch/touch_diff.txt"
#define TOUCH_BANK_DUMP_FILE "/data/log/touch/touch_bank.txt"
...
struct himax_ts_data
{
	...
	// cable status work
	struct work_struct cable_status_work;
...
unsigned char c22[] = { 0xB4, 0x04, 0x01, 0x01, 0x01, 0x01, 0x03, 0x0F, 0x04, 0x07, 0x04,
			0x07, 0x04, 0x07, 0x00};
...
unsigned char c31[] = { 0xC9, 0x00, 0x00, 0x00, 0x00, 0x00, 0x15, 0x15, 0x17, 0x17, 0x19,
			0x19, 0x1F, 0x1F, 0x1B, 0x1B, 0x1D, 0x1D, 0x21, 0x21, 0x23, 0x23,
                        0x25, 0x25, 0x27, 0x27, 0x29, 0x29, 0x2B, 0x2B, 0x2D, 0x2D, 0x2F,
			0x2F, 0x16, 0x16, 0x18, 0x18, 0x1A, 0x1A, 0x20, 0x20, 0x1C, 0x1C,
                        0x1E, 0x1E, 0x22, 0x22, 0x24, 0x24, 0x26, 0x26, 0x28, 0x28, 0x2A,
			0x2A, 0x2C, 0x2C, 0x2E, 0x2E, 0x30, 0x30, 0x00, 0x00, 0x00};
unsigned char c32[] = { 0xCB, 0x01, 0xF5, 0xFF, 0xFF, 0x01, 0x00, 0x05, 0x00, 0x9F, 0x00,
			0x00, 0x00};
unsigned char c33[] = { 0xD0, 0x06, 0x01};
unsigned char c34[] = { 0xD3, 0x06, 0x01};
unsigned char c35[] = { 0xD5, 0xA5, 0x02, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
			0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
unsigned char c36[] = { 0x40, 0x01, 0x5A, 0x77, 0x02, 0xF0, 0x13, 0x00, 0x00, 0x56, 0x10,
			0x14, 0x18, 0x06, 0x10, 0x0C, 0x0F, 0x0F, 0x0F, 0x52, 0x34, 0x80,
                        0x80, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};//start:0x00 ,size 31
unsigned char c37[] = { 0x40, 0xA5, 0x00, 0x80, 0x82, 0x85, 0x00, 0x35, 0x25, 0x0F, 0x0F,
			0x83, 0x3C, 0x00, 0x00, 0x11, 0x11, 0x00, 0x00, 0x01, 0x01, 0x00,
			0x0A, 0x00, 0x00, 0x10, 0x02, 0x10, 0x64, 0x00, 0x00}; // start 0x1E :size 31
unsigned char c38[] = {	0x40, 0x40, 0x38, 0x38, 0x02, 0x14, 0x00, 0x00, 0x00, 0x04, 0x03,
			0x12, 0x06, 0x06, 0x00, 0x00, 0x00}; // start:0x3C ,size 17
unsigned char c39[] = {	0x40, 0x18, 0x18, 0x05, 0x00, 0x00, 0xD8, 0x8C, 0x00, 0x00, 0x42,
			0x03, 0x00, 0x00, 0x00, 0x00, 0x00, 0x10, 0x02, 0x80, 0x00, 0x00,
			0x00, 0x00, 0x0C}; //start 0x4C,size 25
unsigned char c40[] = {	0x40, 0x10, 0x12, 0x20, 0x32, 0x01, 0x04, 0x07, 0x09, 0xB4, 0x6E,
			0x32, 0x00, 0x0F, 0x1C, 0xA0, 0x16, 0x00, 0x00, 0x04, 0x38, 0x07,
			0x80}; //start 0x64,size 23
unsigned char c41[] = {	0x40, 0x03, 0x2F, 0x08, 0x5B, 0x56, 0x2D, 0x05, 0x00, 0x69, 0x02,
			0x15, 0x4B, 0x6C, 0x05, 0x03, 0xCE, 0x09, 0xFD, 0x58, 0xCC, 0x00,
			0x00, 0x7F, 0x02, 0x85, 0x4C, 0xC7, 0x00};//start 0x7A,size 29
unsigned char c42[] = {	0x40, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}; //start 0x96,size 9
unsigned char c43_1[]={ 0x40, 0x00, 0xFF, 0x15, 0x28, 0x01, 0xFF, 0x16, 0x29, 0x02, 0xFF,
			0x1B, 0x2A, 0x03, 0xFF, 0x1C, 0xFF, 0x04, 0xFF, 0x1D, 0xFF, 0x05,
			0x0F, 0x1E, 0xFF, 0x06, 0x10, 0x1F, 0xFF, 0x07, 0x11, 0x20}; //start 0x9E,size 32
unsigned char c43_2[]={ 0x40, 0xFF, 0x08, 0x12, 0x21, 0xFF, 0x09, 0x13, 0x22, 0xFF, 0x0A,
			0x14, 0x23, 0xFF, 0x0B, 0x17, 0x24, 0xFF, 0x0C, 0x18, 0x25, 0xFF,
			0x0D, 0x19, 0x26, 0xFF, 0x0E, 0x1A, 0x27, 0xFF}; //start 0xBD,size 29
unsigned char c44_1[]={ 0x40, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
			0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
			0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}; //start 0xDA,size 32
...
static unsigned char upgrade_fw_file[32*1024];
static unsigned char upgrade_fw_wintek[]=
{
	#include "20140530_ME572_Wintek_V03_No_Glove.h"
};
static unsigned char upgrade_fw_laibo[]=
{
	#include "20140530_ME572_Laibao_V03_No_Glove.h"
};
...
static ssize_t himax_chip_enable_irq(struct device *dev, struct device_attribute *attr, const char *buf, size_t count);
static ssize_t himax_get_fw_version(struct device *dev, struct device_attribute *attr, char *buf);
static ssize_t himax_debug_log(struct device *dev, struct device_attribute *attr, const char *buf, size_t count);
static ssize_t himax_checksum(struct device *dev, struct device_attribute *attr, char *buf);
static ssize_t himax_fw_upgrade(struct device *dev, struct device_attribute *attr, const char *buf, size_t count);
...
static DEVICE_ATTR(tp_check_running, S_IRUGO, himax_chip_check_running, NULL);
static DEVICE_ATTR(touch_irq, S_IWUSR, NULL, himax_chip_enable_irq);
static DEVICE_ATTR(tp_fw_version, S_IRUGO, himax_get_fw_version, NULL);
static DEVICE_ATTR(debug_log, S_IWUSR, NULL, himax_debug_log);
static DEVICE_ATTR(himax_check_sum, S_IRUGO,  himax_checksum, NULL);
...
static void himax_ic_package_check(struct himax_ts_data *ts_modify)
{
	uint8_t cmd[3];
	uint8_t data[3];
	...
	if((data[0] == 0x85 && data[1] == 0x30))
	{
		...
		printk("[Himax] Himax IC package 8530 E\n");
	}
...
static int himax_ts_poweron(void)
{
	...
	printk(KERN_ERR "[Himax] himax_ts_poweron \n");
	mutex_lock(&himax_data->mutex_lock);

	//----[ HX_85XX_C_SERIES_PWON]----start
	if (IC_TYPE == HX_85XX_C_SERIES_PWON)
	{
		printk(KERN_ERR "[Himax] HX_85XX_C_SERIES_PWON!!!!!!!");
	...
	{
		printk(KERN_ERR "[Himax] HX_85XX_A_SERIES_PWON!!!!!!!");
		buf0[0] = HX_CMD_MANUALMODE;
		buf0[1] = 0x02;
		ret = i2c_himax_master_write(himax_data->client, buf0, 2, DEFAULT_RETRY_CNT);//Reload Disable
		if(ret < 0)
	...
	{
		printk(KERN_ERR "[Himax] HX_85XX_B_SERIES_PWON!!!!!!!");
		buf0[0] = HX_CMD_MANUALMODE;
		buf0[1] = 0x02;
		ret = i2c_himax_master_write(himax_data->client, buf0, 2, DEFAULT_RETRY_CNT);//Reload Disable
		if(ret < 0)
	...
	{
		printk(KERN_ERR "[Himax] HX_85XX_D_SERIES_PWON!!!!!!!");
	...
	else if (IC_TYPE == HX_85XX_E_SERIES_PWON)
	{
		...
		buf0[11] = 0x00;
		buf0[12] = 0x00;
		ret = i2c_himax_master_write(himax_data->client, buf0, 13, DEFAULT_RETRY_CNT);
		if(ret < 0)
		{
			printk(KERN_ERR "i2c_master_send failed addr = 0x%x\n",himax_data->client->addr);
			goto send_i2c_msg_fail;
		...
		buf0[0] = 0x83;
		ret = i2c_himax_master_write(himax_data->client, buf0, 1, DEFAULT_RETRY_CNT);
		if(ret < 0)
		{
			printk(KERN_ERR "i2c_master_send failed addr = 0x%x\n",himax_data->client->addr);
			goto send_i2c_msg_fail;
...
static uint8_t himax_calculateChecksum(char *ImageBuffer, int fullLength)//, int address, int RST)
{
	...
	if(IC_CHECKSUM == HX_TP_BIN_CHECKSUM_SW)
	{
		...
		for (i = 0; i < FileLength; i++)
		{
			...
			else
			{
				...
				//Check Success
				if (upgrade_fw_laibo[fullLength - 1] == (u8)(0xFF & (checksum >> 8)) && upgrade_fw_laibo[fullLength - 2] == (u8)(0xFF & checksum))
				{
					himax_FlashMode(0);
	...
	else if(IC_CHECKSUM == HX_TP_BIN_CHECKSUM_CRC)
	{
		...
		if(IC_TYPE == HX_85XX_E_SERIES_PWON)
		{
			...
			if( i2c_himax_read(himax_data->client, 0xED, cmd, 5, DEFAULT_RETRY_CNT) < 0)
			{
				printk(KERN_ERR "[Himax] %s: i2c access fail!\n", __func__);
				return -1;
			}
			...
			if( i2c_himax_write(himax_data->client, 0xED ,&cmd[0], 5, DEFAULT_RETRY_CNT) < 0)
			{
				printk(KERN_ERR "[Himax] %s: i2c access fail!\n", __func__);
				return 0;
			}
			...
			//Select CRC Mode
			cmd[0] = 0x05;
			if( i2c_himax_write(himax_data->client, 0xD2 ,&cmd[0], 1, DEFAULT_RETRY_CNT) < 0)
			{
				printk(KERN_ERR "[Himax] %s: i2c access fail!\n", __func__);
				return 0;
			}
			...
			//Enable CRC Function
			cmd[0] = 0x01;
			if( i2c_himax_write(himax_data->client, 0x53 ,&cmd[0], 1, DEFAULT_RETRY_CNT) < 0)
			{
				printk(KERN_ERR "[Himax] %s: i2c access fail!\n", __func__);
				return 0;
			}
...
static int himax_read_flash(unsigned char *buf, unsigned int addr_start, unsigned int length) //OK
{
	...
	if( i2c_himax_write_command(himax_data->client, 0xAA, DEFAULT_RETRY_CNT) < 0)
	{
...
static u8 himax_read_fw_ver(bool hw_reset)
{
	...
	if( i2c_himax_write(himax_data->client, 0xAA ,&cmd[0], 0, DEFAULT_RETRY_CNT) < 0)
	{
		printk(KERN_ERR "[Himax] %s: i2c access fail!\n", __func__);
		goto err_finish;
	}
...
static ssize_t himax_get_fw_version(struct device *dev, struct device_attribute *attr, char *buf)
{
	printk(KERN_INFO "[Himax] D%s.%s\n",himax_data->himax_firmware_version, himax_data->config_firmware_version);
	...
	for (i = 0, prePage = 0; i < FileLength; i++)
	{
		...
		if (last_byte == 1)
		{
			...
			if (i == (FileLength - 1))
			{
				himax_FlashMode(0);
				himax_ManualMode(0);
				checksumResult = himax_calculateChecksum(fw, fullFileLength);//, address, RST);
				//himax_ManualMode(0);
				himax_lock_flash();
...
static ssize_t himax_debug_level_dump(struct device *dev,struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if(buf[0] == 't')
	{
		...
		result=filp->f_op->read(filp,upgrade_fw_file,sizeof(upgrade_fw_file), &filp->f_pos);
		if(result < 0)
		{
		...
		printk(KERN_INFO "[Himax] %s: upgrade start,len %d: %02X, %02X, %02X, %02X.\n", __func__, result, upgrade_fw_file[0], upgrade_fw_file[1], upgrade_fw_file[2], upgrade_fw_file[3]);
		printk(KERN_INFO "[Himax] %s: upgrade version: %02X %02X.\n", __func__, upgrade_fw_file[0x0085], upgrade_fw_file[0x0086]);
		printk(KERN_INFO "[Himax] %s: last 4 byte, %02X, %02X, %02X, %02X.\n", __func__, upgrade_fw_file[0x7FFC], upgrade_fw_file[0x7FFD], upgrade_fw_file[0x7FFE], upgrade_fw_file[0x7FFF]);

		if(result > 0)
		{
			// start to upgrade
			disable_irq(himax_data->irq);
			if(fts_ctpm_fw_upgrade_with_sys_fs(upgrade_fw_file, result) == 0)
			{
...
firmware_upgrade_done:
	...
	//----[ENABLE_CHIP_RESET_MACHINE]----start
	queue_delayed_work(himax_data->himax_wq, &himax_data->himax_chip_reset_work, 0);
	printk(KERN_INFO "[Himax] %s: himax_chip_reset_work in firmware_upgrade_done \n", __func__);
	//----[ENABLE_CHIP_RESET_MACHINE]----end
...
#ifdef ENABLE_CHIP_STATUS_MONITOR
	queue_delayed_work(himax_data->himax_wq, &himax_data->himax_chip_monitor, 10*HZ);
	printk(KERN_INFO "[Himax] %s: himax_chip_monitor in firmware_upgrade_done \n", __func__);
#endif
	//----[ENABLE_CHIP_STATUS_MONITOR]----end
...
static ssize_t himax_diag_dump(struct device *dev,struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (buf[0] == '1')	//IIR
	{
		if(IC_TYPE == HX_85XX_E_SERIES_PWON)
		{
			...
			i2c_himax_write(himax_data->client, command_F1h[0] ,&command_F1h[1], 1, DEFAULT_RETRY_CNT);
		}
		else
		{
...
static ssize_t himax_chip_raw_data_store(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	if(diag_command == 1)
	{
		...
		if(IS_ERR(filp))
		{
			printk(KERN_ERR "[Himax] %s: open /data/log/touch/touch_dc.txt failed\n", __func__);
			return 0;
		}
		oldfs = get_fs();
		set_fs(get_ds());
	...
	else if(diag_command == 2)
	{
		...
		if(IS_ERR(filp))
		{
			printk(KERN_ERR "[Himax] %s: open /data/log/touch/touch_diff.txt failed\n", __func__);
			return 0;
		}
		oldfs = get_fs();
		set_fs(get_ds());
	...
	else if(diag_command == 3)
	{
		...
		if(IS_ERR(filp))
		{
			printk(KERN_ERR "[Himax] %s: open /data/log/touch/touch_bank.txt failed\n", __func__);
			return 0;
		}
		oldfs = get_fs();
		set_fs(get_ds());
...
static void himax_ts_flash_work_func(struct work_struct *work)
{
	...
	if( i2c_himax_master_write(himax_data->client, xAA_command, 1, 3) < 0 )//sleep out
	{
		printk(KERN_ERR "[Himax] %s: i2c write AA fail.\n",__func__);
	...
	else if(local_flash_command == 4)
	{
		...
		{
			x35_command[1] = 0x01;
			if( i2c_himax_write(himax_data->client, x35_command[0],&x35_command[1], 1, DEFAULT_RETRY_CNT) < 0 )
			{
				printk(KERN_ERR "[Himax] %s: i2c write 35 fail.\n",__func__);
				goto Flash_Dump_i2c_transfer_error;
			}
		}
		else
...
static int himax_chip_self_test(uint8_t *data)
{
	...
	else if(IC_TYPE == HX_85XX_E_SERIES_PWON)
	{
		...
		i2c_himax_write(himax_data->client, 0x80,&cmdbuf[0], 0, DEFAULT_RETRY_CNT);
		msleep(120);

		cmdbuf[0] = 0xA5;
		i2c_himax_write(himax_data->client, 0xCA,&cmdbuf[0], 1, DEFAULT_RETRY_CNT);
		msleep(100);

		cmdbuf[0] = 0xCA;
		i2c_himax_write(himax_data->client, 0xCE,&cmdbuf[0], 1, DEFAULT_RETRY_CNT);
		msleep(100);

		//Step 1 : Close Re-Calibration FE02
		//-->Read 0xFE02
		cmdbuf[0] = 0x15;
		i2c_himax_write(himax_data->client, 0x8C,&cmdbuf[0], 1, DEFAULT_RETRY_CNT);
		msleep(10);

		cmdbuf[0] = 0x00;
		cmdbuf[1] = 0x02; //FE02
		i2c_himax_write(himax_data->client, 0x8B,&cmdbuf[0], 2, DEFAULT_RETRY_CNT);
		msleep(10);
		...
		//Step 5 : Enter self_test mode
		cmdbuf[0] = 0x06;//0x16;
		i2c_himax_write(himax_data->client, 0xF1,&cmdbuf[0], 1, DEFAULT_RETRY_CNT);

		i2c_himax_read(himax_data->client, 0xF1, valuebuf, 1, DEFAULT_RETRY_CNT);
		...
		i2c_himax_write(himax_data->client, 0x81,&cmdbuf[0], 0, DEFAULT_RETRY_CNT);
...
static int update_touch_result(int result)
{
	...
	loff_t pos = 0;
	int len = 0;
	char result_progress[3];

	sprintf(result_progress, "%d", result);
	...
	if(!IS_ERR_OR_NULL(resultfilePtr))
	{
		...
		set_fs(get_ds());
		pos = 0;
		len = resultfilePtr->f_op->write(resultfilePtr, result_progress, sizeof(char), &pos);
		set_fs(oldfs_result);
...
{
	int i = 0;
	bool FW_VER_MAJ_update  = false;
	bool FW_VER_MIN_update  = false;
	bool CFG_VER_MAJ_update = false;
	bool CFG_VER_MIN_update = false;
	int upgrade_status = 0;
	unsigned char* ImageBuffer = NULL;

	if ( (tp_pcbid & 0x2) == 0 )
		ImageBuffer = upgrade_fw_wintek;
	else
		ImageBuffer = upgrade_fw_laibo;

	if (FW_VER_MAJ_buff[0] == ImageBuffer[FW_VER_MAJ_FLASH_ADDR])
		FW_VER_MAJ_update = false;
	else
		FW_VER_MAJ_update = true;

	if (FW_VER_MIN_buff[0] == ImageBuffer[FW_VER_MIN_FLASH_ADDR])
		FW_VER_MIN_update = false;
	else
		FW_VER_MIN_update = true;

	for (i=0; i<12; i++)
	{
		if(CFG_VER_MAJ_buff[i] != ImageBuffer[CFG_VER_MAJ_FLASH_ADDR+i])
			CFG_VER_MAJ_update = true;
	}
	...
	upgrade_status =  FW_VER_MAJ_update | FW_VER_MIN_update
			| CFG_VER_MAJ_update | CFG_VER_MIN_update ;
	...
	{
		printk("[Himax] No need upgrade touch firmware.\n");
		return 1;
	}
}
...
static int himax_firmware_upgrade(int path)
{
	...
	if(upgrade_status)
	{
		...
		// start to upgrade
		disable_irq(himax_data->irq);
		if(fts_ctpm_fw_upgrade_with_sys_fs(ImageBuffer, sizeof(upgrade_fw_wintek)) == 0)
		{
			printk(KERN_INFO "[Himax] %s: firmware upgrade error.\n", __func__);
			fw_update_complete = false;
		...
		{
			printk(KERN_INFO "[Himax] %s: firmware upgrade success. \n", __func__);
			fw_update_complete = true;
	...
	{
		printk(KERN_INFO "[Himax] %s: No need to upgrade firmware.\n", __func__);
	}
...
static void himax_chip_self_firmware_upgrade(struct work_struct *dat)
{
...
//
//==============================================================================
static void cable_status_work_func(struct work_struct *work)
{
	uint8_t buf0[2] = {0};
	...
	{
		printk("[Himax] %s: cable_status=%d, tp_status=%d, init_success=%d.\n",__func__, cable_status, himax_data->tp_status, himax_data->init_success);
		if((himax_data->tp_status == 1) && (himax_data->init_success == 1))
		{
			if( (cable_status == 1) || (cable_status == 2) || (cable_status == 4)) // with charging current
			{
				buf0[0] = 0x01;
				i2c_himax_write(himax_data->client, 0xF0 ,&buf0[0], 1, DEFAULT_RETRY_CNT);
			}
			else// no charging current
			{
				buf0[0] = 0x00;
				i2c_himax_write(himax_data->client, 0xF0 ,&buf0[0], 1, DEFAULT_RETRY_CNT);
			}
...
static int himax_cable_status_notify(struct notifier_block *nb, unsigned long cable_type, void *dev)
{
	printk("[Himax] %s: cable_status = %lu \n", __func__, cable_type);

	cable_status = cable_type;
	queue_work(himax_data->himax_wq, &himax_data->cable_status_work);

	return 0;
}
EXPORT_SYMBOL(himax_cable_status_notify);
...
static int himax_ts_probe(struct i2c_client *client,const struct i2c_device_id *id)
{
	...
	// workqueue for cable status
	INIT_WORK(&himax_data->cable_status_work, cable_status_work_func);
	...
	// register for cable status
	cable_status_register_client(&usb_status_notifier);

	tp_pcbid = asustek_get_tp_type();
	printk("[Himax] tp_pcbid is %d \n", tp_pcbid);
	#ifdef ENABLE_SELF_FIRMWARE_UPGRADE
	queue_delayed_work(himax_data->himax_wq, &himax_data->himax_chip_firmware_upgrade_work, 0);
	#endif

	// check cable status after reset
	himax_cable_status_notify(NULL, query_cable_status(), &client->dev);
...
static int himax_ts_remove(struct i2c_client *client)
{
	...
	cable_status_unregister_client(&usb_status_notifier);
...
{
	printk("[Himax] himax_module_init \n");
	printk(KERN_INFO "[Himax] hx8528 init %s\n", __func__);

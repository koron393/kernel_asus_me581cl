...
/* T71 Dynamic Controller */
#define MXT_T71_SSINTTHR	52
#define MXT_T71_NSINTTHR	72
#define MXT_T71_VNSINTTHR	92
	...
	if(input_dev != NULL)
        {
	        ...
	        input_sync(input_dev);
                ...
                pr_info("[ATMEL]%s:input device is null\n", __func__);
}
...
static int mxt_check_reg_init(struct mxt_data *data)
{
	...
	if (!config_mem) {
        ...
        //the "cfg->size-2" for ignore the last two bytes of return symbol.
	...
	dev_info(dev, "Config written, Checksum:%06X\n",data->config_crc);
...
static int mxt_read_info_block(struct mxt_data *data)
{
	...
	if (error) {
        ...
        /*read t100 intthr and t47 byte0 for ESD workaround*/
	error = __mxt_read_reg(client, data->T100_address + MXT_T100_INTTHR , sizeof(data->t100_init_intthr), &data->t100_init_intthr);
	if (error)
		goto err_free_mem;

	error = __mxt_read_reg(client, data->T47_address + MXT_T47_CTRL, sizeof(data->t47_byte0), &data->t47_byte0);
	if (error)
		goto err_free_mem;
...
{
	gpio_request(data->pdata->gpio_reset, "atmel_mxt_rst");
...
{
	pr_info("[ATMEL_MXT_TS] enter mxt_initialize\n");
	struct i2c_client *client = data->client;
	int error;
...
retry_info:
	pr_debug("[ATMEL_MXT_TS] mxt_initialize_1\n");
...
retry_bootloader:
	...
	pr_debug("[ATMEL_MXT_TS] mxt_initialize_2\n");
	error = mxt_check_retrigen(data);
	if (error)
		return error;

	pr_debug("[ATMEL_MXT_TS] mxt_initialize_3\n");
	error = mxt_acquire_irq(data);
	if (error)
		return error;

	pr_debug("[ATMEL_MXT_TS] mxt_initialize_4\n");
	...
	pr_info("[ATMEL_MXT_TS] mxt_initialize_complete\n");
	return 0;
}
...
*/
	...
	if (data->T9_reportid_min) {
		error2 = mxt_initialize_t9_input_device(data);
		if (error2)
			return error2;
	} else if (data->T100_reportid_min) {
		error2 = mxt_initialize_t100_input_device(data);
		if (error2)
			return error2;
	} else {
				   ...
				   struct device_attribute *attr, char *buf)
{
        struct mxt_data *data = dev_get_drvdata(dev);
        if(data->in_bootloader)
	        return scnprintf(buf, PAGE_SIZE, "0.5.00\n");
        else
				   ...
				   struct device_attribute *attr, char *buf)
{
	struct mxt_data *data = dev_get_drvdata(dev);
        if(data->in_bootloader)
	        return scnprintf(buf, PAGE_SIZE, "164.11\n");
        else
...
/* sensor vendor is returned as tpid0.tpid1*/
static ssize_t mxt_tp_vendor_show(struct device *dev,
				   struct device_attribute *attr, char *buf)
{
        int tpid0 = 0;
        int tpid1 = 0;
        /*ID-0(gpio160) for sensor vendor,
          ID-1(gpio161) for BM color
          id_0 id_1
             0    0:SR2 FPC extension code:1
             0    1:O-film code:2
             1    0:JTouch white code:3
             1    1:JTouch black code:4
	*/
        tpid0 = gpio_get_value(160);
        tpid1 = gpio_get_value(161);

        if(tpid0 == 0 && tpid1 == 0)
                return scnprintf(buf, PAGE_SIZE, "%d\n",1);
        else if(tpid0 == 0 && tpid1 != 0)
                return scnprintf(buf, PAGE_SIZE, "%d\n",2);
        else if(tpid0 != 0 && tpid1 == 0)
                return scnprintf(buf, PAGE_SIZE, "%d\n",3);
        else if(tpid0 != 0 && tpid1 != 0)
				   ...
				   struct device_attribute *attr, char *buf)
{
	...
	if (!obuf)
                return scnprintf(buf, PAGE_SIZE, "%d\n", version_int);
        //assign object to T38
	object = data->object_table+5;
	if (!object)
...
static ssize_t mxt_enable_pen_store(struct device *dev,
		struct device_attribute *attr,
		const char *buf, size_t count)
{
	struct mxt_data *data = dev_get_drvdata(dev);
...
static ssize_t mxt_pen_mode_show(struct device *dev,
				   struct device_attribute *attr, char *buf)
{
        struct mxt_data *data = dev_get_drvdata(dev);
        int error;
        u8 t47_control;
	error = __mxt_read_reg(data->client, data->T47_address + MXT_T47_CTRL, sizeof(t47_control), &t47_control);
	if (error)
		return -EIO;
        if((t47_control&1) == 0)
                return scnprintf(buf, PAGE_SIZE, "0\n");
...
static DEVICE_ATTR(tp_vendor, S_IRUGO, mxt_tp_vendor_show, NULL);
static DEVICE_ATTR(cfg_version, S_IRUGO, mxt_config_version_show, NULL);
static DEVICE_ATTR(cfg_name_update, S_IWUSR | S_IRUSR, mxt_cfg_name_show, mxt_update_cfg_name_store);
static DEVICE_ATTR(enable_pen, S_IWUSR | S_IRUGO, mxt_pen_mode_show, mxt_enable_pen_store);
...
static void mxt_start(struct mxt_data *data)
{
	...
	if (data->use_regulator) {
		...
		mxt_set_t7_power_cfg(data, MXT_POWER_CFG_RUN);
	        pr_info("[ATMEL_MXT_TS] TS_RESUME.\n");

		/* Recalibrate since chip has been in deep sleep */
		mxt_t6_command(data, MXT_COMMAND_CALIBRATE, 1, false);
	        pr_info("[ATMEL_MXT_TS] TS_RESUME_CALIBRATE.\n");
	}
...
static void mxt_stop(struct mxt_data *data)
{
	...
	if (data->use_regulator)
        ...
        pr_info("[ATMEL_MXT_TS] TS_SUSPEND.\n");
...
{
	pr_info("[ATMEL_MXT_TS] Enter probe!!\n");
	...
	client->addr = 0x4a;
	pr_debug("[ATMEL_MXT_TS] probe_function_1\n");
	...
	pr_debug("[ATMEL_MXT_TS] probe_function_2\n");
	snprintf(data->phys, sizeof(data->phys), "i2c-%u-%04x/input0",
		 client->adapter->nr, client->addr);

	pr_debug("[ATMEL_MXT_TS] probe:client->irq=%d\n",client->irq);
	client->irq = gpio_to_irq(116);

	data->client = client;
	data->irq = client->irq;
	i2c_set_clientdata(client, data);

	pr_debug("[ATMEL_MXT_TS] probe:data->irq=%d\n",data->irq);
	pr_debug("[ATMEL_MXT_TS] probe_function_3\n");
	error = mxt_handle_pdata(data);
	if (error)
		goto err_free_mem;

	rst_gpio = data->pdata->rst_gpio;
	int_gpio = data->pdata->int_gpio;
	data->pdata->gpio_reset = 191;
	data->pdata->irqflags = IRQF_TRIGGER_FALLING | IRQF_ONESHOT;

	pr_debug("[ATMEL_MXT_TS] probe: rst_gpio=%d, int_gpio=%d\n",rst_gpio,int_gpio);
	...
	//data->irq = gpio_to_irq(int_gpio);
	//pr_debug("[ATMEL_MXT_TS] probe:data->irq=%d\n",data->irq);
	pr_debug("[ATMEL_MXT_TS] probe: pdata->gpio_reset=%d\n",data->pdata->gpio_reset);
	pr_debug("[ATMEL_MXT_TS] probe_function_4\n");
	...
	if(gpio_request(rst_gpio, "atmel_mxt_rst") != 0)
		pr_info("[ATMEL_MXT_TS] probe: reset gpio request failed\n");

	gpio_direction_output(rst_gpio, 0);

	if(gpio_request(int_gpio, "atmel_mxt_intr") != 0)
		pr_info("[ATMEL_MXT_TS] probe: reset gpio request failed\n");

	pr_debug("[ATMEL_MXT_TS] probe_function_5\n");
	mxt_probe_regulators(data);

	pr_debug("[ATMEL_MXT_TS] probe_function_6\n");
	disable_irq(data->irq);

	pr_debug("[ATMEL_MXT_TS] probe_function_7\n");
	error = mxt_initialize(data);
	if (error)
		goto err_free_irq;

	pr_debug("[ATMEL_MXT_TS] probe_function_8\n");
	...
	pr_debug("[ATMEL_MXT_TS] probe_function_9\n");
	...
	pr_debug("[ATMEL_MXT_TS] probe_function_10\n");
	...
	pr_info("[ATMEL_MXT_TS] probe_function_complete\n");

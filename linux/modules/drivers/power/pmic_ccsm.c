...
#ifdef PMIC_A0_WAR
#define PMIC_APQ_AP_ACOK 179
static struct workqueue_struct *pmic_gpio_acok_wq;
#endif

#define SP_USB_PLUG 56
static u8 temp_chgrirq1_int;
static u8 temp_chgrirq1_stat;

#ifdef CONFIG_SLIMPORT_ANX7814
...
#ifdef CONFIG_SLIMPORT_ANX7814
	printk(KERN_INFO "%s connected slimport dongle %d\n", __func__, slimport_is_connected()? 1 : 0);
#endif
...
#ifdef CONFIG_SLIMPORT_ANX7814
	if (gpio_get_value(PMIC_APQ_AP_ACOK) && slimport_is_connected()) {
		mutex_lock(&chc.evt_queue_lock);
		chc.vbus_connect_status = false;
		mutex_unlock(&chc.evt_queue_lock);
...
static void handle_level1_interrupt(u8 int_reg, u8 stat_reg)
{
	...
	if (vendor_id == SHADYCOVE_VENDORID) {
		...
		if (int_reg & CHRGRIRQ1_SUSBIDGNDDET_MASK) {
			if (mask) {
				if (asustek_get_project_id() == 2 || asustek_get_project_id() == 3) {
					gpio_direction_output(SP_USB_PLUG, 0);
				} else {
					gpio_direction_output(SP_USB_PLUG, 1);
				}
				...
				if (asustek_get_project_id() == 0 || asustek_get_project_id() == 7) {
	...
	if ((vendor_id == BASINCOVE_VENDORID) && (int_reg & CHRGRIRQ1_SUSBIDDET_MASK)) {
		if (mask) {
			if (asustek_get_project_id() == 2 || asustek_get_project_id() == 3) {
				gpio_direction_output(SP_USB_PLUG, 1);
			} else {
	...
	if (int_reg & CHRGRIRQ1_SVBUSDET_MASK) {
		mask = stat_reg & SCHRGRIRQ1_SVBUSDET_MASK;
		if (mask) {
			dev_info(chc.dev,
		...
		printk(KERN_INFO "%s, otg_mode_enabled = %d\n", __func__, chc.otg_mode_enabled? 1 : 0);
		printk(KERN_INFO "%s, mask = %d\n", __func__, mask);
		...
		if (asustek_get_project_id() == 2 || asustek_get_project_id() == 3) {
			...
			/* Avoid charger-detection flow in case of host-mode */
			if (chc.is_internal_usb_phy && !chc.otg_mode_enabled && !gpio_get_value(PMIC_APQ_AP_ACOK))
				handle_internal_usbphy_notifications(mask);
			else {
				if (!gpio_get_value(PMIC_APQ_AP_ACOK))
					atomic_notifier_call_chain(&chc.otg->notifier, USB_EVENT_VBUS, &mask);
				if (!mask)
					chc.otg_mode_enabled = false;
			}
...
static irqreturn_t pmic_gpio_acok_isr(int irq, void *dev_id)
{
	...
	if ((gpio_get_value(PMIC_APQ_AP_ACOK)? 1 : 0))
		...
		queue_delayed_work(pmic_gpio_acok_wq, &chc.slimport_connect_vbus_work, 0.5*HZ);
#endif
...
#ifdef CONFIG_SLIMPORT_ANX7814
static void slimport_connect_vbus_work_function(struct work_struct *work)
{
	printk(KERN_INFO "%s connected slimport dongle %d\n", __func__, slimport_is_connected()? 1 : 0);
	if ((slimport_is_connected()? 1 : 0)) {
		handle_level1_interrupt(temp_chgrirq1_int, temp_chgrirq1_stat);
	}
}
#endif
...
{
	int err = 0 ;
	unsigned gpio = PMIC_APQ_AP_ACOK;
	...
	printk(KERN_INFO "GPIO pin irq %d requested ok, PMIC_APQ_AP_ACOK = %s\n", irq_num, gpio_get_value(gpio)? "H":"L");
	return 0;
}
#endif
...
static void pmic_event_worker(struct work_struct *work)
{
	...
	list_for_each_entry_safe(evt, tmp, &chc.evt_queue, node) {
		...
		temp_chgrirq1_int = evt->chgrirq1_int;
		temp_chgrirq1_stat = evt->chgrirq1_stat;
		if (evt->chgrirq0_int)
...
static irqreturn_t pmic_thread_handler(int id, void *data)
{
	...
	if (asustek_get_project_id() == 2 || asustek_get_project_id() == 3)
		queue_delayed_work(system_nrt_wq, &chc.evt_work, 0*HZ);
	else
		queue_delayed_work(system_nrt_wq, &chc.evt_work, 1.0*HZ);
...
static int pmic_check_initial_events(void)
{
	...
	if (evt->chgrirq1_stat || evt->chgrirq0_int) {
		...
		list_add_tail(&evt->node, &chc.evt_queue);
		mutex_unlock(&chc.evt_queue_lock);
		schedule_delayed_work(&chc.evt_work, 1.0*HZ);
	}
...
static int pmic_chrgr_probe(struct platform_device *pdev)
{
	...
	retval = gpio_request(gpio, "SP_USB_PLUG");
	if (retval)
		printk(KERN_INFO "gpio %d request failed\n", SP_USB_PLUG);
...
#ifdef CONFIG_SLIMPORT_ANX7814
	INIT_DELAYED_WORK(&chc.slimport_connect_vbus_work, slimport_connect_vbus_work_function);
#endif

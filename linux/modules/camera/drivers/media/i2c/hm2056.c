...
#define PROJECT_ID_ME572C	0x2
#define PROJECT_ID_ME572CL	0x3

#define to_hm2056_raw_sensor(sd) container_of(sd, struct hm2056_raw_device, sd)

//Add for i2c test+++
#define HM2056_IOC_MAGIC 0xF5
#define HM2056_IOC_MAXNR 2
#define HM2056_POLL_DATA _IOR(HM2056_IOC_MAGIC,2,int )

#define HM2056_IOCTL_START_HEAVY 2
#define HM2056_IOCTL_START_NORMAL 1
#define HM2056_IOCTL_END 0

#define START_NORMAL    (HZ)
#define START_HEAVY     (HZ)

static void hm2056_stress_test_poll(struct work_struct * work);
static int stress_test_poll_mode=0;
struct delayed_work hm2056_stress_test_poll_work;
static struct workqueue_struct *hm2056_stress_test_work_queue;
struct i2c_client *hm2056_client;
static int hm2056_stress_test_power_on(struct i2c_client *client);
static int hm2056_stress_test_power_off(struct i2c_client *client);
//Add for i2c test---
...
//Add for i2c test+++
static int hm2056_open(struct inode *inode, struct file *file)
{
        printk("%s\n", __func__);
        return 0;
}

static int hm2056_release(struct inode *inode, struct file *file)
{
        printk("%s\n", __func__);
        return 0;
}

static long hm2056_ioctl_test(struct file *file, unsigned int cmd, unsigned long arg)
{
        int err = 1, ret = 0;
        if (_IOC_TYPE(cmd) != HM2056_IOC_MAGIC)
                return -ENOTTY;
        if (_IOC_NR(cmd) > HM2056_IOC_MAXNR)
                return -ENOTTY;
        ...
        switch (cmd) {
                case HM2056_POLL_DATA:
                        if (arg == HM2056_IOCTL_START_HEAVY){
                                printk("%s ioctl heavy\n", __func__);
                                ret = hm2056_stress_test_power_on(hm2056_client);
                                stress_test_poll_mode = START_HEAVY;
                                queue_delayed_work(hm2056_stress_test_work_queue, &hm2056_stress_test_poll_work, stress_test_poll_mode);
                        }
                        else if (arg == HM2056_IOCTL_START_NORMAL){
                                printk("%s ioctl normal\n", __func__);
                                ret = hm2056_stress_test_power_on(hm2056_client);
                                stress_test_poll_mode = START_NORMAL;
                                queue_delayed_work(hm2056_stress_test_work_queue, &hm2056_stress_test_poll_work, stress_test_poll_mode);
                        }
                        else if  (arg == HM2056_IOCTL_END){
                                printk("%s ioctl end\n", __func__);
                                cancel_delayed_work_sync(&hm2056_stress_test_poll_work);
                                ret = hm2056_stress_test_power_off(hm2056_client);
                        }
                        else
                                return -ENOTTY;
                        break;
...
// ASUS_BSP ---, define for i2c stress test
//Add for i2c test---
			      ...
			      struct v4l2_mbus_framefmt *fmt)
{
	...
	pr_info("%s(%d): write %s\n", __func__, __LINE__,
		dev->hm2056_raw_res[dev->fmt_idx].desc);
...
//Add for i2c test+++
static int hm2056_stress_test_detect(struct hm2056_raw_device *dev, struct i2c_client *client)
{
	struct i2c_adapter *adapter = client->adapter;
	u32 reg_h, reg_l;
	u32 chip_id;
...
//Add for i2c test
static const struct file_operations hm2056_fops = {
        .owner = THIS_MODULE,
        .open = hm2056_open,
        .release = hm2056_release,
        .unlocked_ioctl = hm2056_ioctl_test,
};

static struct miscdevice hm2056_compass_dev = {
...
static int hm2056_raw_remove(struct i2c_client *client)
{
	...
	//Add for i2c test+++
	printk("%s\n", __func__);
	if (misc_deregister(&hm2056_compass_dev) < 0) {
		dev_err(&client->dev, "misc deregister hm2056_compass_dev failed.");
	}
	//Add for i2c test---
		       ...
		       const struct i2c_device_id *id)
{
	...
	switch(project_id) {
		...
		case PROJECT_ID_ME572CL:
		break;

		default:
			pr_info("hm2056 is unsupported on this board(0x%x)\n",
				project_id);
		return -EINVAL;
	}
#endif
...
//Add for i2c test+++
        ret = misc_register(&hm2056_compass_dev);
        if (ret) {
                dev_err(&client->dev, "%s: hm2056_compass_dev register failed", __func__);
                goto destroy_stress_test_wq;
        }

        hm2056_client = client;
        hm2056_stress_test_work_queue = create_singlethread_workqueue("i2c_camera_wq");
        if(!hm2056_stress_test_work_queue){
                printk("camera hm2056 : unable to create i2c stress test workqueue\n");
                goto destroy_stress_test_wq;
        }
        INIT_DELAYED_WORK(&hm2056_stress_test_poll_work, hm2056_stress_test_poll);
//Add for i2c test---
...
destroy_stress_test_wq:
        destroy_workqueue(hm2056_stress_test_work_queue);
        return ret;
//Add for i2c test---
}
...
static int hm2056_stress_test_power_on(struct i2c_client *client)
{
        ...
        ret = hm2056_raw_s_power(sd, 1);
        if (ret) {
                v4l2_err(client, "hm2056 stress power-up err");
                return ret;
        }
...
static int hm2056_stress_test_power_off(struct i2c_client *client)
{
        ...
        ret = hm2056_raw_s_power(sd, 0);
        if (ret) {
                v4l2_err(client, "hm2056 stress power down err");
                return ret;
        }
...
static void hm2056_stress_test_getid(struct i2c_client *client)
{
        struct hm2056_raw_device *dev;
        struct v4l2_subdev *sd = i2c_get_clientdata(client);
        int ret = 0;
        ...
        /* config & detect sensor */
        ret = hm2056_stress_test_detect(dev, client);
        if (ret) {
                v4l2_err(client, "hm2056_stress_getId err.\n");
        }
}
...
static void hm2056_stress_test_poll(struct work_struct * work)
{
        ...
        hm2056_stress_test_getid(hm2056_client);

...
#define PROJECT_ID_ME581CL	0x0
#define PROJECT_ID_ME581C	0x7
			       ...
			       struct atomisp_exposure *exposure)
{
	 ...
	 */
	/*
	 * Hindden Register usage in REG_SW_RESET bit 15.
	 * set REG_SW_RESET bit 15 as 1 for group apply.
...
static int
mt9m114_g_frame_interval(struct v4l2_subdev *sd,
...
/* ++++ dbgfs ++++ */
static int dbg_mt9m114_vga_status_open(
...
static ssize_t dbg_mt9m114_vga_status_read(
	struct file *file,
	char __user *buf,
	size_t count,
	loff_t *ppos)
{
	...
	/* Read MT9M114 Chip ID */
	mt9m114_read_reg(g_client, MISENSOR_16BIT, (u32)MT9M114_PID, &chip_id);

	if (chip_id == MT9M114_MOD_ID) {
...
static const struct file_operations dbg_mt9m114_vga_status_fops = {
	.open		= dbg_mt9m114_vga_status_open,
	.read		= dbg_mt9m114_vga_status_read,
};

static int mt9m114_dbgfs_init(void)
{
	struct dentry *debugfs_dir;

	debugfs_dir = debugfs_create_dir("mt9m114", NULL);

	(void) debugfs_create_file("vga_status", S_IRUGO,
		debugfs_dir, NULL, &dbg_mt9m114_vga_status_fops);
		       ...
		       const struct i2c_device_id *id)
{
	...
	switch(project_id) {
		...
		case PROJECT_ID_ME581C:
		break;
		default:
			pr_info("mt9m114 is unsupported on this board(0x%x)\n",
				project_id);
		return -EINVAL;
	}
	...
	if (client->dev.platform_data) {
		...
		if (ret) {
			...
			kfree(dev);
			pr_info("%s(%d)-- mt9m114_s_config err\n",
					__func__, __LINE__);
			return ret;
		}
	}
	...
	if (ret) {
		...
		pr_info("%s(%d)-- media_entity_init err\n", __func__, __LINE__);
		return ret;
	}

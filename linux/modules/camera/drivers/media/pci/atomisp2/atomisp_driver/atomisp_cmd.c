...
static void ia_print_csi_rx_errors(struct atomisp_device *isp, unsigned int irq)
{
	if (irq <= 0)
		return;

	dev_err(isp->dev, "CSI Receiver errors: (irq = 0x%x)\n", irq);

	if (irq & CSS_RX_IRQ_INFO_SINGLE_BIT_PACKET_HEADER_ERR)
		dev_err(isp->dev, "  Single bit packet header error corrected");
	if (irq & CSS_RX_IRQ_MULTI_BIT_PACKET_HEADER_ERR)
		dev_err(isp->dev, "  Multiple bit packet header errors detected");
	if (irq & CSS_RX_IRQ_PAYLOAD_CRC_ERR)
		dev_err(isp->dev, "  Payload checksum (CRC) error");
	if (irq & CSS_RX_IRQ_FIFO_OVERFLOW)
		dev_err(isp->dev, "  FIFO overflow");
	if (irq & CSS_RX_IRQ_RESERVED_SHORT_PACKET_DATA_TYPE)
		dev_err(isp->dev, "  Reserved short packet data type detected");
	if (irq & CSS_RX_IRQ_RESERVED_LONG_PACKET_DATA_TYPE)
		dev_err(isp->dev, "  Reserved long packed data type detected");
	if (irq & CSS_RX_IRQ_INCOMPLETE_LONG_PACKET_ERR)
		dev_err(isp->dev, "  Incomplete long packet detected");
	if (irq & CSS_RX_IRQ_FRAME_SYNC_ERROR)
		dev_err(isp->dev, "  Frame sync error");
	if (irq & CSS_RX_IRQ_LINE_SYNC_ERROR)
		dev_err(isp->dev, "  Line sync error");
	if (irq & CSS_RX_IRQ_DPHY_SOT_ERR)
		dev_err(isp->dev, "  DPHY start of transmission error");
	if (irq & CSS_RX_IRQ_DPHY_SYNC_ERR)
		dev_err(isp->dev, "  DPHY synchronization error");
	if (irq & CSS_RX_IRQ_ESC_MODE_ERR)
		dev_err(isp->dev, "  Escape mode error");

#if 0 /* not recognize as error */
	if (irq & CSS_RX_IRQ_ESC_MODE_TRIGGER_EVENT)
		dev_err(isp->dev, "  Escape mode trigger event");
	if (irq & CSS_RX_IRQ_ESC_MODE_ULPS_FOR_DATA)
		dev_err(isp->dev, "  Escape mode ULPS for data lane");
	if (irq & CSS_RX_IRQ_ESC_MODE_ULPS_EXIT_FOR_CLOCK)
		dev_err(isp->dev, "  Escape mode ULPS exit for clock lane");
#endif
...
irqreturn_t atomisp_isr(int irq, void *dev)
{
	...
	/* CSI RX IRQ A */
	irq0 = ia_css_device_load_uint32((hrt_address) 0xC1408) & 0x0FFF;
	irq1 = ia_css_device_load_uint32((hrt_address) 0xC3408) & 0x0FFF;

#if 0
	dev_err(isp->dev, " isys irq status irq0=0x%x, irq1=0x%x  (ignore)\n",
		ia_css_device_load_uint32((hrt_address) 0xC1408),
		ia_css_device_load_uint32((hrt_address) 0xC3408));
#endif

	if (irq0 || irq1) {
		dev_err(isp->dev, "isys irq status irq0=0x%x, irq1=0x%x\n",
			irq0, irq1);
		ia_print_csi_rx_errors(isp, irq0);
		ia_print_csi_rx_errors(isp, irq1);
			 ...
			 struct atomisp_dvs2_bq_resolutions *bq_res)
{
	...
	if (!asd->continuous_mode->val) {
		...
		if (IS_HWREVISION(asd->isp, ATOMISP_HW_REVISION_ISP2401)) {
			bq_res->ispfilter_bq.width_bq = 16 / 2;
			bq_res->ispfilter_bq.height_bq = 16 / 2;
		} else {
		...
		unsigned int dvs_env_w, unsigned int dvs_env_h)
{
	...
	pr_info("%s(%d): s_mbus_fmt: ask %ux%u (padding %ux%u, dvs %ux%u)\n",
		__func__, __LINE__,
	...
	if (source_pad == ATOMISP_SUBDEV_PAD_SOURCE_VF ||
		...
		if (source_pad == ATOMISP_SUBDEV_PAD_SOURCE_PREVIEW &&
			...
			if (!asd->yuvpp_mode) {
				...
				atomisp_css_preview_enable_online(asd, false);
			}

				 ...
				 enum ia_css_pipe_id pipe_id)
{
	 ...
	 */
	printk("online %d continuous %d raw_binning %d\n",
		stream_config->online, stream_config->continuous, pipe_extra_configs->enable_raw_binning);
	...
	printk("bayer_out %d x %d\n", bayer_ds_out_res->width, bayer_ds_out_res->height);
	/*
	...
	printk("yuv_pp_in %d x %d\n", vf_pp_in_res->width, vf_pp_in_res->height);

	if ((out_width == 176) && (out_height == 144)) {
		printk("%s(): ignore 176x144 yuv decimation\n", __func__);
	} else {
...
{
	printk("%s\n", __func__);
	asd->params.config.ext_nr_config = ext_nr_config;
}

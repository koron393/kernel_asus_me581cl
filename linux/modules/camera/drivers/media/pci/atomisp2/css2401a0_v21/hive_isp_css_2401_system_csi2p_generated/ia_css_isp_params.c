...

	{
		...
		if (offset >= 0) {
			ia_css_bnr_encode((struct sh_css_isp_bnr_params *)
					&stage->binary->mem_params.params[IA_CSS_PARAM_CLASS_PARAM][IA_CSS_ISP_DMEM].address[offset],
					&params->nr_config);
			ia_css_ext_bnr_encode((struct sh_css_isp_bnr_params *)
					&stage->binary->mem_params.params[IA_CSS_PARAM_CLASS_PARAM][IA_CSS_ISP_DMEM].address[offset],
					&params->ext_nr_config);
			params->isp_params_changed = true;
			params->isp_mem_params_changed[pipe_id][stage->stage_num][IA_CSS_ISP_DMEM] = true;
		}
	...
	struct ia_css_isp_parameters *params)
{
	...
	{
		short offset = stage->binary->info->mem_offsets.offsets.param->dmem.formats;
	...
	struct ia_css_isp_parameters *params)
{
	...
	{
		short offset = stage->binary->info->mem_offsets.offsets.param->dmem.output;
...

	{
		...
		if (offset >= 0) {
			ia_css_nr_encode((struct sh_css_isp_ynr_params *)
					&stage->binary->mem_params.params[IA_CSS_PARAM_CLASS_PARAM][IA_CSS_ISP_DMEM].address[offset],
					&params->nr_config);
			ia_css_ext_nr_encode((struct sh_css_isp_ynr_params *)
					&stage->binary->mem_params.params[IA_CSS_PARAM_CLASS_PARAM][IA_CSS_ISP_DMEM].address[offset],
					&params->ext_nr_config);
			params->isp_params_changed = true;
			params->isp_mem_params_changed[pipe_id][stage->stage_num][IA_CSS_ISP_DMEM] = true;
...
void
ia_css_set_ext_nr_config(struct ia_css_isp_parameters *params,
			const struct ia_css_ext_nr_config *config)
{
	...
	assert(params != NULL);
	ia_css_debug_dtrace(IA_CSS_DEBUG_TRACE, "ia_css_set_ext_nr_config() enter:\n");
	ia_css_ext_nr_debug_dtrace(config, IA_CSS_DEBUG_TRACE);
	params->ext_nr_config = *config;

	ia_css_debug_dtrace(IA_CSS_DEBUG_TRACE_PRIVATE, "ia_css_set_ext_nr_config() leave: "
		"return_void\n");
}
		...
		const struct ia_css_isp_config *config)
{
	...
	ia_css_set_ext_nr_config(params, config->ext_nr_config);

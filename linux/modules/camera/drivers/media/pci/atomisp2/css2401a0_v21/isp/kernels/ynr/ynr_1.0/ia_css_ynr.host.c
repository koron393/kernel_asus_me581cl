...
const struct ia_css_ext_nr_config default_ext_nr_config = {
	16384,
	16384
};
...
static bool ext_nr_config_changed;

void
ia_css_ext_ynr_enable(bool enable)
{
	ext_nr_config_changed = enable;
}
	...
	const struct ia_css_nr_config *from)
{
	...
	if (ext_nr_config_changed == false)
		to->gain_dir =
		    uDIGIT_FITTING(from->ynr_gain, 16, SH_CSS_YNR_GAIN_SHIFT);
	to->threshold_cb =
...
void
ia_css_ext_nr_encode(struct sh_css_isp_ynr_params *to,
	const struct ia_css_ext_nr_config *from)
{
	/* YNR (Y Noise Reduction) */
	if (ext_nr_config_changed == true)
		to->gain_dir =
		    uDIGIT_FITTING(from->ynr_gain_dir, 16, SH_CSS_YNR_GAIN_SHIFT);
}
...
void
ia_css_ext_nr_debug_dtrace(const struct ia_css_ext_nr_config *config, unsigned level)
{
	ia_css_debug_dtrace(level,
